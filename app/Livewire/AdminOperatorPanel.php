...
class AdminOperatorPanel extends Component
{
    ...
    public function takeover(int $id, OperatorService $ops): void
    {
        ...
        $me = auth()->user();
        if (!$vc || !$me) return;
        try { $ops->takeover($vc, $me); $this->status = "Takeover VC #$id."; }
    ...
    public function pause(int $id, OperatorService $ops): void
    {
        ...
        $me = auth()->user();
        if (!$vc || !$me) return;
        try { $ops->pause($vc, $me); $this->status = "Paused VC #$id."; }
        catch (\Throwable $e) { $this->status = $e->getMessage(); }
    }
    ...
    public function resume(int $id, OperatorService $ops): void
    {
        ...
        $me = auth()->user();
        if (!$vc || !$me) return;
        try { $ops->resume($vc, $me); $this->status = "Resumed VC #$id."; }
        catch (\Throwable $e) { $this->status = $e->getMessage(); }
    }
    ...
    {
        $items = collect();
        try { $items = VirtualConversation::latest('id')->take(20)->get(); } catch (\Throwable) {}
