...
class ProfileCompleteness extends Component
{
    public function render()
    {
        ...
        if ($u && $u->relationLoaded('profile') === false) {
            try {
                $u->loadMissing('profile');
            } catch (\Throwable) {
            }
        }
        if ($u && $u->profile) {
            try {
                $pct = max($pct, $u->profile->completenessScore());
            } catch (\Throwable) {
            }
        }
        $tips = [];
        if ($u) {
            ...
            if (empty($u->profile?->bio)) {
                $tips[] = 'Tulis bio menarik';
            }
            if (empty($u->profile?->occupation)) {
                $tips[] = 'Isi pekerjaan';
            }
            if (($u->interests()->count() ?? 0) === 0) {
