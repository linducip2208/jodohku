...
<a class="jk-side-link {{ request()->is('forums*') || request()->is('blog*') || request()->is('komunitas*') ? 'active' : '' }}" href="/komunitas"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M21 12a8 8 0 0 1-8 8H4l2-3a8 8 0 1 1 15-5z"/><path d="M8 10h8M8 14h5"/></svg> Komunitas</a>
...
<button class="jk-navlink {{ request()->is('matches*') || request()->is('biro-jodoh*') || request()->is('forums*') || request()->is('blog*') || request()->is('events*') || request()->is('notifications*') || request()->is('settings*') || request()->is('profile*') ? 'active' : '' }}" @click="$store.more.open = true" aria-label="Menu lainnya" aria-haspopup="dialog" style="background:none;border:0;cursor:pointer;font-family:inherit"><svg class="ico" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="5" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="19" cy="12" r="1.6"/></svg>Lainnya</button>
</nav>
