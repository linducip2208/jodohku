...
@if($src)<img src="{{ $src }}" alt="Foto {{ $name }}" style="width:100%;height:100%;object-fit:cover;display:block" draggable="false">@else<div class="jk-photo-fallback">{{ strtoupper(substr((string)$name,0,1)) }}</div>@endif
@if(count($images) > 1)
...
@foreach($images as $i => $im)<div style="flex:1;height:3px;border-radius:2px;background:{{ $i <= $photoIndex ? '#fff' : 'rgba(255,255,255,.45)' }}"></div>@endforeach
</div>
@endif
@if($cand->compatibility_score ?? null)<span class="jk-compat">{{ (int) $cand->compatibility_score }}% cocok</span>@endif
...
@if($cand->is_verified)<span class="jk-badge-verified" title="Terverifikasi" aria-label="Terverifikasi"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="10" fill="currentColor"/><path d="M8 12.5l2.7 2.7L16.5 9" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>@endif</div>
<div class="jk-muted">{{ $cand->city ?? 'Indonesia' }}@if($distance !== null) · {{ $distance }} km@endif · {{ $cand->is_online ? 'Online sekarang' : 'Terakhir aktif '.($cand->last_active_at?->diffForHumans() ?? '—') }}</div>
@if($cand->profile?->headline)<p style="margin:8px 0 0;font-size:14px">{{ \Illuminate\Support\Str::limit($cand->profile->headline, 90) }}</p>@endif
</div>
</div>
</div>
...
<button class="jk-btn jk-btn-pass" wire:click="pass" title="Lewati (geser kiri / ←)">✕</button>
<button class="jk-btn jk-btn-super" wire:click="superlike" title="Super Like">✦</button>
<button class="jk-btn jk-btn-like" wire:click="like" title="Suka (geser kanan / →)">❤️</button>
</div>
...
@if($status)<div class="jk-muted" style="text-align:center;font-size:12px" role="status">{{ $status }}</div>@endif
@if($matchedUserId)
