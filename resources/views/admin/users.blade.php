...
<tr><td>{{ $u->id }}</td><td>{{ $u->displayName() }} @if($u->is_verified)✔@endif</td><td>{{ $u->email }}</td><td>{{ $u->account_type?->value ?? $u->account_type }}</td><td>{{ $u->status?->value ?? $u->status }}</td><td>{{ $u->is_premium ? '⭐' : '—' }}</td>
