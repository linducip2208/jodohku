...
<tr><td>{{ $p->user?->displayName() ?? $p->user_id }}</td><td>{{ \Illuminate\Support\Str::limit($p->headline ?? '', 40) }}</td><td>{{ $p->occupation }}</td><td>{{ $p->education }}</td><td>{!! $p->is_complete ? '<span class="badge bg-green"><i class="ti ti-check"></i> Lengkap</span>' : '<span class="badge">—</span>' !!}</td></tr>
@endforeach
</tbody></table></div></div>
@endsection
