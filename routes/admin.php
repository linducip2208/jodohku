...
Route::prefix('admin')->name('admin.')->middleware(['web', 'auth', 'active.account'])->group(function () {
    ...
    // ---- Whitelabel brands: admins manage all, clients only their own
    // (BrandPolicy scopes per-action; create/delete/import stay admin-only).
    Route::middleware('can:brand-manager')->group(function () {
        ...
        Route::post('/brands/{brand}/verify-domain', [BrandController::class, 'verifyDomain'])->name('brands.verify')->middleware('throttle:5,1,brand-admin');
